...
/*
   Special thanks to Alamo for authoring this addition to the flight software!
   ...
   It's a good idea to use an external 5V linear regulator, such as an LM7805/LM317 or similar to power to the MAX7456. The MAX7456 can draw up to 100mA @ 5V
    - this would result in an extra ~0.7W being dissipated across the onboard regulator (for a 3S battery). That's a lot of power for a little regulator with minimal heatsinking!
*/

#define ShowReticle
#define ShowFlightTimer //Displays how long the motors have been armed for
#define ShowAttitudeIndicator
//#define NTSC          //Comment this line out for PAL, uncomment it for NTSC
...
public:
  ...
  void initialize( void ) {
    int i;
    pinMode( CS, OUTPUT );
    pinMode( 53, OUTPUT ); //SPI transfers block if this pin isn't set as output (this is the default CS pin)
...
private:
  ...
  //Clears a column col of some characters - clears centreRow+-offset
  ...
  void updateAltitude(void) {
    ...
    writeChars( buf, strlen((char*)buf)+1, 2, 0 );
...
void updateTimer(void) {
  ...
  writeChars( (byte*) timerAscii, 6, 0, 23 );
}
#endif
