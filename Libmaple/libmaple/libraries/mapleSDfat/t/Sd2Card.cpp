...
uint8_t Sd2Card::waitNotBusy(uint16_t timeoutMillis)
{
  ...
  do
  {
	  loop++;
    if (spiRec() == 0XFF) {
    	SerialDebug.print("waitNotBusy loop ");
    	SerialDebug.print(loop);
    	SerialDebug.print(" time ");
    	SerialDebug.println(millis() - t0);
		return true;
    }
  }
  while (((uint16_t)millis() - t0) < timeoutMillis);
	SerialDebug.print("waitNotBusy failed loop ");
	SerialDebug.print(loop);
	SerialDebug.print(" time ");
	SerialDebug.println(millis() - t0);
  return false;
