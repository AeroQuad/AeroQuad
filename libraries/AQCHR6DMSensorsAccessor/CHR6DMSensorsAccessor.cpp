...
boolean CHR6DM::syncToHeader()  
{
  ...
  {
    if (blockingRead()==PACKET_HEADER[0] && blockingRead()==PACKET_HEADER[1] && blockingRead()==PACKET_HEADER[2] )
    {
      return true;
    }
  }
...
boolean CHR6DM::requestPacket()
{
  sendPacket(GET_DATA);
}

boolean CHR6DM::waitForAndReadPacket()
{
...
boolean CHR6DM::setListenMode() 
{
  sendPacket(SET_SILENT_MODE);
  return waitForAck(DEFAULT_TIMEOUT);
}

boolean CHR6DM::waitForAck(int timeout) 
{
  long startTime = millis();
  while(millis()-startTime<timeout)
  {
