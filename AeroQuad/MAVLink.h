...
#if defined(UseGPSNavigator)
		__mavlink_mission_count_t waypointCount;
		mavlink_msg_mission_count_decode(&msg, &waypointCount);

		if (waypointCount.count > MAX_WAYPOINTS) {
			...
			waypointCount.count = MAX_WAYPOINTS;
		}

		waypointsToBeRequested = waypointCount.count;
