...
void handleMessage(mavlink_message_t msg) {
	...
	case MAVLINK_MSG_ID_COMMAND_LONG:
		...
		case MAV_CMD_PREFLIGHT_CALIBRATION: //241, calibration of acc/gyro/mag/transmitter
			if (!motorArmed) {
				...
				if (commandPacket.param6 == 1.0f) {
					...
					mavlink_msg_statustext_send(chan, MAV_SEVERITY_INFO, "EEPROM initalized");
					// Suppress command ack message to prevent the statustext above from disappearing
					suppressCommandAckMsg = true;
				}
		...
		if (!suppressCommandAckMsg) {
			mavlink_msg_command_ack_send(chan, commandPacket.command, result);
		}
		break;
	...
	case MAVLINK_MSG_ID_PARAM_SET:  // set one specific onboard parameter
		if (!motorArmed) { // added for security reason, as the software is shortly blocked by this command
			...
			mavlink_msg_statustext_send(chan, MAV_SEVERITY_ERROR, "Only allowed with disarmed motors!");
		}
		break;

	case MAVLINK_MSG_ID_MISSION_REQUEST_LIST:  // requesting complete waypoint list from AQ
#if defined(UseGPSNavigator)
		mavlink_msg_mission_count_send(chan, MAV_SYSTEM_ID, MAV_COMPONENT_ID, waypointCount);

		waypointTimeLastSent = millis();
		waypointSendingToGCS = true;
...
#if defined(UseGPSNavigator)
		__mavlink_mission_count_t waypointCountPacket;
		mavlink_msg_mission_count_decode(&msg, &waypointCountPacket);

		if (waypointCountPacket.count > MAX_WAYPOINTS) {
			...
			waypointCountPacket.count = MAX_WAYPOINTS;
		}

		waypointsToBeRequested = waypointCountPacket.count;
		waypointTimeLastReceived = millis();
		waypointReceivingFromGCS = true;
		waypointSendingToGCS = false;
	...
	case MAVLINK_MSG_ID_MISSION_CLEAR_ALL:  // clear waypoint list of AQ
		...
		for (byte index = 0; index < MAX_WAYPOINTS; index++) {
			...
			waypoint[index].altitude = GPS_INVALID_ALTITUDE;
		}
...
void readSerialCommand() {
	...
	if (waypointSendingToGCS && (tnow - waypointTimeLastSent) > waypointSendTimeout) {
		waypointSendingToGCS = false;
	}
