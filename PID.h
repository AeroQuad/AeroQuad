...
float updatePIDangle(float targetPosition, float currentPosition, float gyroData, struct PIDdata *PIDparameters) {
  ...
  angleControl = (PIDparameters->P * error) + (PIDparameters->I * (PIDparameters->integratedError)); // Angle PI Control
  angleRate = angleControl - gyroData; // Rate D Control
  
  return angleRate * PIDparameters->D;
  ...
  //return (PIDparameters->P * error) + (PIDparameters->I * (PIDparameters->integratedError)) + (gyroData * PIDparameters->D);
}
