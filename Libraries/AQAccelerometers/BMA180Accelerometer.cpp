...
void BMA180Accelerometer::initialize() 
{
  // Check if accel is connected
  if (readWhoI2C(_accelAddress) != 0x03) // page 52 of datasheet
  {
  ...
  updateRegisterI2C(_accelAddress, 0x10, 0xB6); //reset device
  ...
  updateRegisterI2C(_accelAddress, 0x0D, 0x10); //enable writing to control registers
  sendByteI2C(_accelAddress, 0x20); // register bw_tcs (bits 4-7)
  byte data = readByteI2C(_accelAddress); // get current register value
  updateRegisterI2C(_accelAddress, 0x20, data & 0x0F); // set low pass filter to 10Hz (value = 0000xxxx)
  ...
  //  8.0g = 0.99 mg/LSB
  // 16.0g = 1.98 mg/LSB
  sendByteI2C(_accelAddress, 0x35); // register offset_lsb1 (bits 1-3)
  data = readByteI2C(_accelAddress);
  data &= 0xF1; // +/-1.0g (value = xxxx000x) // 0xF7;(3g)  //0xF5; (2g)
  updateRegisterI2C(_accelAddress, 0x35, data);
}
  
void BMA180Accelerometer::measure() 
{
  int rawData[3];
  ...
  Wire.requestFrom(_accelAddress, 6);
...
const int BMA180Accelerometer::getFlightData(byte axis) 
{
  return getRaw(axis) >> 4;
}
  
// Allows user to zero accelerometers on command
void BMA180Accelerometer::calibrate() 
{  
  ...
  for (byte calAxis = ROLL; calAxis < ZAXIS; calAxis++) 
  {
    ...
    {
      sendByteI2C(_accelAddress, dataAddress);
      findZero[i] = readReverseWordI2C(_accelAddress) >> 2; // last two bits are not part of measurement
      delay(1);
    }
