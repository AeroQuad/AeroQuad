...
void WiiAccelerometer::measure() 
{
  ...
  {
    _accelADC[axis] = _accelZero[axis] - _wiiSensorAccessor->getAccelerometerValue(axis);
...
void WiiAccelerometer::calibrate() 
{
  ...
  for(byte calAxis = ROLL; calAxis < LASTAXIS; calAxis++) 
  {
    for (int i=0; i<FINDZERO; i++) 
    {
      ...
      findZero[i] = _wiiSensorAccessor->getAccelerometerValue(calAxis);
    }
