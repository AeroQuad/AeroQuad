...
void Kinematics_MARG::margUpdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, unsigned long G_Dt) {
  float norm;
...
// @see Kenny, weird Arduino bug when declared here, ala42, help!!!   
/*void Kinematics_MARG::eulerAngles(void) {
...
void Kinematics_MARG::initialize(float hdgX, float hdgY) 
{
...
void Kinematics_MARG::calculate(float rollRate,          float pitchRate,    float yawRate,       \
  ...
  margUpdate(rollRate,          pitchRate,    yawRate, \
             ...
             measuredMagX,      measuredMagY, measuredMagZ, G_Dt);
  eulerAngles();
}
  
float Kinematics_MARG::getGyroUnbias(byte axis) {
