...
unsigned char genPoly[NPAR+1] = {0x25,0xe0,0x08,0xac,0x47,0xb2,0x2c,0xe3,0x01};
...
void
mult_polys (unsigned char dst[], unsigned char p1[], unsigned char p2[])
{
  short i, j;
  unsigned char tmp1[MAXDEG*2];
...
void copy_poly (unsigned char dst[], unsigned char src[]) 
{
  unsigned char i;
  for (i = 0; i < MAXDEG; i++) dst[i] = src[i];
}

void zero_poly (unsigned char poly[]) 
{
  unsigned char i;
...
main (int argc, char *argv[])
{
  ...
  /* Encode data on codeword, adding NPAR parity bytes */
  memcpy(codeword,msg,sizeof(msg));

  encode_data(codeword, sizeof(msg));
 

  printf("With parity:\n");
  hexdump(codeword,sizeof(msg) + NPAR);
