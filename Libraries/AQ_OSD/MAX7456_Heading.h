...
void displayHeading(byte row, byte col, boolean redraw, float currentHeading) {

  float deg = degrees(currentHeading);
  int currentHeadingDeg = (int)( 0.5 + (deg<-0.5 ? deg+360.0 : deg));
  static int lastHeading;

  if (redraw) lastHeading=361;

  if (currentHeadingDeg!=lastHeading) {
    char buf[6];
    snprintf(buf,6,"\026%3d\027",currentHeadingDeg);
    writeChars( buf, 5, 0, row, col );
